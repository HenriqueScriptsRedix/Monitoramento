...
# CorePath pode estar em 64 ou WOW6432Node
$regPaths = @(
...
function Resolve-RepositoryObjectFromJob {
    ...
    foreach ($name in 'RepositoryId','TargetRepositoryId') {
        $repo = Find-RepositoryById -Id (Get-SafeMemberValue -Object $Job -Name $name)
        if ($repo) { return $repo }
    }
...
function Get-ObjectStorageLabel {
    ...
    foreach ($value in @($repoName, $repoType, $typeDisplay)) {
        if ($value -match 'S3|AWS') { return 'Amazon S3' }
    }
...
foreach ($PLUGINJOB in $PLUGINJOBS) {
    ...
    if ($null -ne $JOBSESSION) {
        ...
        try { $StorageDetail = Resolve-StorageDetailFromJob -Job $PLUGINJOB } catch {}
